...
func (s *apigw) Reload(ctx context.Context) (err error) {
	...
	{
		var (
			defaultMethodResponse = helperMethodNotAllowed(s.opts)
			defaultResponse       = helperDefaultResponse(s.opts)
			profiler              = helperProfiler(s.opts, s.pr)
		)

		s.mx.NotFound(profiler(defaultResponse))
		s.mx.MethodNotAllowed(profiler(defaultMethodResponse))
	}
...
func (s *apigw) Init(ctx context.Context, routes ...*route) {
	...
	for _, r := range s.routes {
		var (
			...
			// pipe = pipeline.NewPipeline(log, chain.NewProfiler())
		)
